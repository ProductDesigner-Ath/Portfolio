...
export const usePortfolioAnimations = (container: React.RefObject<HTMLDivElement | null>) => {
  useEffect(() => {
    window.gsap = gsap;
  ...
  void main(){
    ...
    gl_FragColor=vec4(r,g,b,uAlpha);
        ...
        function openProject(id: string) {
          if (id === 'p5') {
            document.body.classList.add('ramdoot-route-out');
            window.setTimeout(() => { window.location.href = '/case-studies/ramdoot'; }, 380);
            return;
          }
          const { p } = photoEls[id];
          projectOpen = true;
          ...
          gsap.to(allPhotos, {
            opacity: 0, y: -20, duration: 0.45, ease: 'power2.in', stagger: { amount: 0.3, from: 'random' },
            onComplete: () => {
              ...
              gsap.to(projectView, {
                opacity: 1, duration: 0.45, ease: 'power2.out', onComplete: () => {
        ...
        const projCloseX = document.getElementById('proj-close-x');
        if (projCloseX) projCloseX.addEventListener('click', closeProject);
        ...
        /* FILTER LOGIC MOVED TO Filter.tsx */
